...
export async function guardarContacto(valor: Contacto) {
  const db = supabaseAdmin();
  const { error } = await db.from("configuracion_sitio").upsert({ clave: "contacto", valor, actualizado_en: new Date().toISOString() });
  if (error) throw error;
  revalidatePath("/admin/configuracion");
